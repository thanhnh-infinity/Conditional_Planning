...
Get help/report bugs via : https://potassco.org/support
    ...
    def run(self):
        ...
        #print("--- QASP Paser time : " + str(currentDT_2 - currentDT_1) + " ms")
        #print("--Thanhnh")
        #print(options)
        return options
    ...
    def smodels2smodels(self, fd):
        ...
        for line in fd:
            ...
            if match:
                ...
                #print("Thanh---------")
                #self.error("{}:{}:{}".format(quant, level, atom))
                if level <= 0:
                    self.error(
                ...
                if pair is not None:
                    if pair[0] != 0 and level != pair[0]:
                        if ERROR_REQUANTIFY:
                            ...
                            if inner % 2 == 0:
                                #self.error("Sao lai vao day")
                                self.unsat = True
                            elif level > pair[0]:
                                level = pair[0]

                    atoms[atom] = (level, pair[1])
                    #self.error(str(level) + ":" + str(pair[1]))
                else:
                    atoms[atom] = (level, 0)
                    #self.error(atoms)
                    #self.error(atoms[atom])
                continue
            ...
            if match:
                ...
                #self.error("{}:{}".format(number, atom))
                #self.error(atoms)
                #key = ""
                #if (key in atoms):
                   #if (key is 'occur(phylotastic_GetAllSpeciesFromTaxon_OT_GET,3)'):
                        #self.error(atoms[key]) 
                   #self.error(check_atom)
                #self.error('occur(convert_gene_tree_format_Phylo4Tree_to_PhyloTree,9)')   
                #self.error(atoms.get('occur(convert_gene_tree_format_Phylo4Tree_to_PhyloTree,9)'))
                #self.error(atom)
                #self.error("|" + str(atom) + "|")
                #self.error(len(atoms))
                pair = atoms.get(atom)
                if pair is not None:
                    #self.error("ABC")
            ...
            for key, (level, number) in atoms.items():
                #if (key[:7] == "h(clean"):
                #if (key[:24] == "exists(phylomatic_method"):
                #if (key[:6] == "exists"):
                    #self.error(self.unsat)
                    #self.error(str(key) + ":" + str(level) + ":" + str(number))
                if level == 0:
        ...
        for line in fd:
            ...
            if state == COMMENTS:
                ...
                if match:
                    ...
                    #self.error(str(number) +":" + str(level) + ":" + str())
                    #self.error(atoms)
                    ...
                    if level % 2 == 1 and level == min_level:
                        ...
                        #self.error(shown[number])
                    #if level % 2 == 0:
                        #self.error(match.group(2) + ":" + match.group(4))      
                    continue
                ...
                #self.error("p cnf {} {}".format(nvars, clauses))
                #print prefix
                string, last_level = "", -1
                for level in keys:
        ...
        for line in fd:
            ...
            if line[0:2] == "V ":
                ...
                for number in line[2:].split():
                    #print("%s : %s" %(str(number),str(shown.get(number))))
...
# MAIN
#
# Sequence function : SMODEL -> Intepret / CNF
